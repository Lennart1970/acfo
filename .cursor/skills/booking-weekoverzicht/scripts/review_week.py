...
def parse_args(argv=None):
    ...
    p.add_argument("--input", default="", help="uploaded Exact transaction .xlsx")
    p.add_argument(
        "--from-sql",
        action="store_true",
        help="read LEDGER_URL (/api/lines) or DATABASE_URL instead of Excel",
    )
    p.add_argument(
        "--week",
        default="",
...
def main(argv=None):
    ...
    if args.from_sql or (not args.input and (os.environ.get("LEDGER_URL") or os.environ.get("DATABASE_URL"))):
        from sql_transactions import load_replica

        bundle = apply_scope(load_replica(), args.scope)
        if args.inspect:
            json.dump(
                {
                    "source": bundle.get("source"),
                    "sourceName": bundle.get("sourceName"),
                    "ready": bool((bundle.get("parse") or {}).get("ok")),
                    "mapped": bundle.get("mappedHeaders") or {},
                    "parse": bundle.get("parse") or {},
                },
                sys.stdout,
                ensure_ascii=False,
        ...
        if not args.input:
            print("pass --input Excel or --from-sql", file=sys.stderr)
            return 2
        if args.inspect:
