...
# Booking check v2 — weekreview

Opvolger van `booking-dagoverzicht`. Geen Exact-API. Geen LedgerBotje.

Twee bronnen, dezelfde score:

1. **SQL-replica** (deploy): Invantive Data Hub → Supabase → Railway `acfo web`. `LEDGER_URL` + `LEDGER_API_KEY`, of `DATABASE_URL`.
2. **Excel-upload** (lokaal / Copilot): Invantive `gmr-eol-transaction-lines.xlsx`.

Patroon: **eerst de bron leesbaar maken, dan week vragen, dan Python-score.**
...
Als `LEDGER_URL` of `DATABASE_URL` gezet is (Railway-webview / Supabase):

```bash
python3 scripts/review_week.py --from-sql --inspect --json
python3 scripts/review_week.py --from-sql --json
python3 scripts/review_week.py --from-sql --week 2026-W23 --output Weekoverzicht-2026-W23.xlsx --json
```

Anders: vraag om een Exact/Invantive-Excel. Zonder bestand én zonder SQL: stop.
...
- `scripts/review_week.py` — CLI (`--inspect`, `--from-sql`, daarna weken, daarna score)
- `scripts/sql_transactions.py` — SQL/API-replica → dezelfde bundle als Excel
