...
skill zip. The sandbox has no outbound network: callers must already have
fetched Exact JSON (LedgerBotje tools or lb_mcp) and pass it as a case file.
"""
...
def score_case(case: dict, config: dict | None = None) -> dict:
    """Score one purchase entry from an already-fetched case file.

    Required keys:
      entry: dict (Exact purchase header)
      ...
      accounts_count: int (accounts_search count for this supplier)
    """
